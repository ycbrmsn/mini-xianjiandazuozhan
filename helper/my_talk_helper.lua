...
function MyTalkHelper.queryFragment (actor)
  ...
  if (info.maxPlayer) then
    table.insert(sessions, TalkSession:reply('目前红队搜集碎片最多的玩家是#G' .. info.maxPlayer))
    table.insert(sessions, TalkSession:reply('嗯，已经搜集了#G' .. info.max .. '#W枚碎片'))
  else
    table.insert(sessions, TalkSession:reply('目前红队还没有人搜集到碎片'))
  end
  info = teamInfos[2]
  if (info.maxPlayer) then
    table.insert(sessions, TalkSession:reply('目前蓝队搜集碎片最多的玩家是#G' .. info.maxPlayer))
    table.insert(sessions, TalkSession:reply('嗯，已经搜集了#G' .. info.max .. '#W枚碎片'))
  else
