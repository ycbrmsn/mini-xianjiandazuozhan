...
function WorldHelper.playOpenDoorSoundOnPos (pos, volume, pitch)
  return WorldHelper.playSoundOnPos(pos, WorldHelper.SOUND_ID.OPEN_DOOR, volume, pitch)
end

-- 在指定位置上播放关门的声音
function WorldHelper.playCloseDoorSoundOnPos (pos, volume, pitch)
  return WorldHelper.playSoundOnPos(pos, WorldHelper.SOUND_ID.CLOSE_DOOR, volume, pitch)
end

function WorldHelper.playBeAttackedSoundOnPos (pos, volume, pitch)
  return WorldHelper.playSoundOnPos(pos, WorldHelper.SOUND_ID.BE_ATTACK, volume, pitch)
end

-- 放置方块的声音
function WorldHelper.playPlaceBlockSoundOnPos (pos, volume, pitch)
  return WorldHelper.playSoundOnPos(pos, WorldHelper.SOUND_ID.ACTION5, volume, pitch)
end

-- 位置上播放声音
function WorldHelper.playSoundOnPos (pos, soundId, volume, pitch, isLoop)
  volume = volume or WorldHelper.volume
  pitch = pitch or WorldHelper.pitch
  WorldHelper.playSoundEffectOnPos(pos, soundId, volume, pitch, isLoop)
end

-- 攻击特效
function WorldHelper.playAttackEffect (pos)
  return WorldHelper.playBodyEffect(pos, WorldHelper.PARTICLE_ID.BOOM1)
end

function WorldHelper.stopAttackEffect (pos)
  return WorldHelper.stopBodyEffect(pos, WorldHelper.PARTICLE_ID.BOOM1)
end

-- 击退特效
function WorldHelper.playRepelEffect (pos)
  return WorldHelper.playBodyEffect(pos, WorldHelper.PARTICLE_ID.BOOM16)
end

function WorldHelper.stopRepelEffect (pos)
  return WorldHelper.stopBodyEffect(pos, WorldHelper.PARTICLE_ID.BOOM16)
end

function WorldHelper.playBodyEffect (pos, particleId, scale)
  scale = scale or 1
  return WorldHelper.playParticalEffect(pos.x, pos.y, pos.z, particleId, scale)
end

function WorldHelper.stopBodyEffect (pos, particleId)
  return WorldHelper.stopEffectOnPosition(pos.x, pos.y, pos.z, particleId)
end
...
function WorldHelper.spawnProjectileByDirPos (shooter, itemid, pos, dirVector3, speed)
  return WorldHelper.spawnProjectileByDir(shooter, itemid, pos.x, pos.y, pos.z,
