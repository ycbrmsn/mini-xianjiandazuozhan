...
function AreaHelper.addToastArea (areaid, info)
  AreaHelper.showToastAreas[areaid] = info
end
...
function AreaHelper.createMovePosArea (pos)
  return AreaHelper.createAreaRect(pos, AreaHelper.movePosDim)
end

-- 创建靠近区域
function AreaHelper.createApproachPosArea (pos)
  return AreaHelper.createAreaRect(pos, AreaHelper.approachPosDim)
end

-- 创建扩大1格区域
function AreaHelper.createNineCubicArea (pos)
  return AreaHelper.createAreaRect(pos, AreaHelper.nineCubicDim)
end

-- 创建自由移动区域
function AreaHelper.createFreeTimeArea (pos)
  return AreaHelper.createAreaRect(pos, AreaHelper.freeTimeDim)
end

-- 创建初始化生物区域
function AreaHelper.createInitActorArea (pos)
  return AreaHelper.createMovePosArea(pos)
end

-- 清空所有木围栏
function AreaHelper.clearAllWoodenFence (areaid)
  return AreaHelper.clearAllBlock(areaid, BlockHelper.woodenFenceid)
end
...
function AreaHelper.getAllCreaturesAndPlayersInAreaId (areaid)
  local posBeg, posEnd = AreaHelper.getAreaRectRange(areaid)
  local objids1 = AreaHelper.getAllCreaturesInAreaRange(posBeg, posEnd)
  local objids2 = AreaHelper.getAllPlayersInAreaRange(posBeg, posEnd)
